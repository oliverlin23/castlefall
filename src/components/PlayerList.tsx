...
interface PlayerListProps {
  ...
  showTeams?: boolean;
  /** Ids currently tracked on the presence channel. Omit to hide connection state. */
...
export function PlayerList({ players, currentPlayerId, showTeams, connectedIds, onKick }: PlayerListProps) {
  return (
    <ul className="divide-y divide-[color:var(--color-ink)]/15 border border-[color:var(--color-ink)] bg-[color:var(--color-paper-bright)]">
      {players.map((player, idx) => {
        ...
        const isAway = connectedIds !== undefined && !connectedIds.has(player.id);
          ...
          >
            ...
            <span
              className={`text-[13px] font-medium flex-1 min-w-0 truncate ${
                isAway ? 'text-[color:var(--color-ink-soft)]' : `text-[color:var(--color-ink)] ${teamColorClass}`
              }`}
            >
              {player.display_name}
            </span>
            {isAway && (
              <span
                className="font-mono text-[9px] uppercase tracking-[0.18em] text-[color:var(--color-ink-soft)] border border-dashed border-[color:var(--color-ink-soft)] px-1.5 py-0.5"
                title="Not connected right now. Their seat is held until they return or are removed."
              >
                away
              </span>
            )}
            ...
            {onKick && !isSelf && (
              <button
                ...
                className={`px-1 py-0.5 text-[color:var(--color-ink-soft)] hover:text-[color:var(--color-team2)] transition-opacity ${
                  isAway ? 'opacity-100' : 'opacity-40 group-hover:opacity-100'
                }`}
